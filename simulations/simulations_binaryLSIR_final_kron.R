...
generateDataLSIR <- function(n,phi,psi,t,p,rho1,rho2,what_time="1:t/t",ind_tpts = FALSE,delta_y = FALSE) {
  ...
  if (delta_y) {
    delta1 <- 0.1*(rho1^abs(outer(1:(t*p), 1:(t*p), FUN = `-`)))
    delta2 <- rho1^abs(outer(1:(t*p), 1:(t*p), FUN = `-`))
  } else {
    delta1 <- delta2 <- kronecker(rho1^abs(outer(1:(p), 1:(p), FUN = `-`)),
                                  rho2^abs(outer(1:(t), 1:(t), FUN = `-`)))
  }
...
simulateSDR <- function(nsim, n, phi,psi, t, p, rho1,rho2,H=1, d=1, k=1, npred = n, what_time = "1:t/t",
  ...
  new_X_red <- X_red 
  AUC_est <- array(c(0),dim=c(5*k + 2+k,2,nsim),dimnames=list(names(X_red),c("logit","logit w.2.ord.terms")))
  ...
  for (i in 1:nsim) {
    ...
    set.seed(i+1000)
    datas <- generateDataLSIR(n,phi,psi,t,p,rho1,rho2,what_time=what_time,ind_tpts=ind_tpts, delta_y=delta_y)
    newdatas <- generateDataLSIR(npred,phi,psi,t,p,rho1,rho2,what_time=what_time,ind_tpts=ind_tpts, delta_y=delta_y)
    ...
    for (j in 1:(5*k + 2 + k)) {
      ...
      # if (dim==1) {
      #   gam_fit <- gam(myform,data = data.frame(X1=t(X_red[[j]]),y=datas$y),family = binomial)
      #   probs_gam <- predict(gam_fit,data.frame(X1=t(new_X_red[[j]])))
      # } else {
      #   gam_fit <- gam(myform,data = data.frame(t(X_red[[j]]),y=datas$y),family = binomial)
      #   probs_gam <- predict(gam_fit,data.frame(t(new_X_red[[j]])))
      # }
      # AUC_est[j,2,i] <- performance(prediction(c(probs_gam),newdatas$y),measure="auc")@y.values[[1]]
    }
  }
  
  return(list(phi=phi,psi=psi,
              yAUC_mean = apply(AUC_est,1:2,mean), yAUC_sd=apply(AUC_est,1:2,sd),
              what_time=what_time
              ))
}
...
#                       n,          phi,     psi,   t, p, H, d, rho1,rho2, k,                time, ind_tpts, delta_y
#              --------------------------------------------------------
params <- list( #list( 500, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 3, "exp(1:t/6-t/6)", FALSE, TRUE),
                list( 500, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 3, "exp(1:t/6-t/6)", FALSE, FALSE),
                #list( 500, c(1,0.5,0.1),(1:8)/4,      8, 3, 1, 4,   0.8,0.5, 3, "runif(t)", TRUE, TRUE),
                #list( 500, c(1,0.5,0.1),(1:8)/4,      8, 3, 1, 4,   0.8,0.5, 3, "runif(t)", TRUE, FALSE),
  #list( 2000, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 1, "exp(1:t/6-t/6)", FALSE, TRUE),
  list( 2000, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 1, "exp(1:t/6-t/6)", FALSE, FALSE),
  #list( 2000, c(1,0.5,0.1),(1:8)/4,      8, 3, 1, 4,   0.8,0.5, 1, "runif(t)", TRUE, TRUE),
  #list( 2000, c(1,0.5,0.1),(1:8)/4,      8, 3, 1, 4,   0.8,0.5, 1, "runif(t)", TRUE, FALSE),
  #list( 500, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 3, "runif(t)", TRUE, TRUE),
  list( 500, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 3, "runif(t)", TRUE, FALSE),
  #list( 2000, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 1, "runif(t)", TRUE, TRUE),
  list( 2000, c(1,0.5,0.1),(1:8)/8,      8, 3, 1, 4,   0.8,0.5, 1, "runif(t)", TRUE, FALSE)
...
for (param in params) {
  ...
  counter <- counter + 1
  c(n,phi,psi,t,p,H,d,rho1,rho2,k,time_sel,ind_tpts,delta_y) %<-% param
  sim <- simulateSDR(nsim, n, phi,psi, t, p, rho1,rho2,H, d,k=k,npred=100,what_time = time_sel,
                     ind_tpts = ind_tpts,delta_y = delta_y)

  saveRDS(sim, file = sprintf("../results/simulation_binaryLSIR_%d_%d_%d_%d_%d_%d_%d_%.2f_%.2f_%d_time_%.1s_ind_tpts_%s_delta_y_%s_kron.rds",
                            nsim, n, round(sum(abs(phi))+sum(abs(psi))), t, p, H, d, rho1,rho2,k,time_sel,ind_tpts,delta_y))
}
