...
simulateSDR_wald <- function(nsim, n, B, t, p, rho,H=1, d=1, what_time = "1:t/t",cvec = 0:5/5,delta_y=FALSE) {
  len_c <- length(cvec) 
  res <- NULL
  rej_rates <- list(STIR=numeric(len_c),logit=numeric(len_c),logit2=numeric(len_c))
  rkwald <- wald <- matrix(c(0),len_c,nsim)
  rkS <- NULL
  rkVCB <- matrix(c(0),len_c,nsim)
  
  for (c_count in 1:len_c) {
    rej_count <- list(STIR=numeric(nsim),logit=numeric(nsim),logit2=numeric(nsim))
    ...
    for (i in 1:nsim) {
      ...
      # summary(fit_logit)
      sub_delta <- res$delta_ls[1:t,]
      logit_test <- crossprod(sub_delta%*%coef(fit_logit)[-1],solve(tcrossprod(sub_delta%*%summary(fit_logit)$cov.unscaled[-1,-1],sub_delta),sub_delta%*%coef(fit_logit)[-1]))
      if (pchisq(logit_test,t,lower.tail = FALSE) <= 0.05) {
        rej_count$logit[i] <- 1
      }
      sub_delta2 <- (mean(datas$y==0)*datas$delta1+mean(datas$y==1)*datas$delta2)[1:t,]
      logit_test2 <- crossprod(sub_delta2%*%coef(fit_logit)[-1],solve(tcrossprod(sub_delta2%*%summary(fit_logit)$cov.unscaled[-1,-1],sub_delta2),sub_delta2%*%coef(fit_logit)[-1]))
      if (pchisq(logit_test2,t,lower.tail = FALSE) <= 0.05) {
        rej_count$logit2[i] <- 1
      }
      
    }
    rej_rates$STIR[c_count] <- mean(rej_count$STIR)
    rej_rates$logit[c_count] <- mean(rej_count$logit)
    rej_rates$logit2[c_count] <- mean(rej_count$logit2)
  }
  return(list(what_time=what_time, cvec = cvec, rej_rates=rej_rates,
              rkwald = rkwald, wald=wald,
              rkS=rkS, rkVCB=rkVCB))
}
...
for (param in params) {
  ...
  saveRDS(sim, file = sprintf("../results/wald_binarySDR_%d_%d_%d_%d_%d_%d_%d_%.2f_time_%.1s_delta_y_%s_maxc_%.2f_delta_beta.rds",
