...
simulateSDR_wald <- function(nsim, n, B, t, p, rho,H=1, d=1, what_time = "1:t/t",cvec = 0:5/5,delta_y=FALSE) {
  ...
  for (c_count in 1:len_c) {
    ...
    for (i in 1:nsim) {
      ...
      dim(X) <- c(t*p, n)
      covX <- cov(t(X))
      sigma_isqrt <- matpow(covX, -0.5)
      Z <- sigma_isqrt %*% X
      dim(Z) <- c(t, p, n)
      
      res <- STIR(X=Z, y=datas$y, t=t, p=p,basis_S = "Polynomial",H=H,d=d,k=NULL,time_pts = datas$time_pts,calc_VC_vecB=TRUE,center_S = TRUE)
      ...
      dim(Z) <- c(t*p,n)
      fit_logit <- glm(y~.,family = binomial(link = "logit"),data = data.frame(t(Z),y=datas$y))
      # summary(fit_logit)
...
for (param in params) {
  ...
  saveRDS(sim, file = sprintf("../results/wald_binarySDR_%d_%d_%d_%d_%d_%d_%d_%.2f_time_%.1s_delta_y_%s_maxc_%.2f_stand.rds",
