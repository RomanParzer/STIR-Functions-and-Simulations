...
generateDataSTIR <- function(n,B,t,p,rho,H=1L,d=1L,what_time = "1:t/t", basis_S = c("Fourier","Polynomial"),
  ...
  X <- sapply(1:n,function(i) {
    ...
    if (y[i]==0) {
      ...
      return(rmvt(1, delta=c(mu), sigma=delta1,df=3))
    } else {
      return(rmvt(1, delta=c(mu), sigma=delta2,df=3))
    }
  })
  dim(X) <- c(t,p,n)
...
for (param in params) {
  ...
  saveRDS(sim, file = sprintf("../results/simulation_binarySTIR_%d_%d_%d_%d_%d_%d_%d_%.2f_%d_time_%.1s_ind_tpts_%s_delta_y_%s_t3dist_err.rds",
