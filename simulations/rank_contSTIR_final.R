...
simulateSDR <- function(nsim, n, B, t, p, rho,H=1, d=1, k=1, npred = 100, what_time = "1:t/t",list_rm_lsir=list(c(1,1)),nlev_ydis = 10,
  ...
  new_X_red <- X_red 
  predmodBIC <- y_mse <- y_cor <- array(c(0),dim=c(5*k + length(list_rm_lsir)+1+k,3,nsim),dimnames=list(names(X_red),c("lm","lm w.2.ord.terms","gam")))
...
for (param in params) {
  ...
  saveRDS(sim, file = sprintf("../results/rank_contSTIR_%d_%d_%d_%d_%d_%d_%d_%.2f_%d_time_%.1s_ind_tpts_%s_delta_y_%s.rds",
