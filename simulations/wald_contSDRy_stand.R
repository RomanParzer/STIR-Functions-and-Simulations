  ...
  for (c_count in 1:len_c) {
    ...
    for (i in 1:nsim) {
      ...
      if (gen_STIR) {
        ...
        res <- STIR(X=Z, y=datas$y, t=t, p=p, d=d,basis_S = "Polynomial",H=H,cont_y = TRUE,k=NULL,time_pts = datas$time_pts,calc_VC_vecB=TRUE)
        ...
        dim(Z) <- c(t*p,n)
        lm_fit <- lm(y~.,data = data.frame(t(Z),y=datas$y))
...
for (param in params) {
  ...
  saveRDS(sim, file = sprintf("../results/wald_contSDR_%d_%d_%d_%d_%d_%d_%d_%.2f_time_%.1s_delta_y_%s_maxc_%.2f_3rdMarker_stand.rds",
