...
generateDataLSIR <- function(n,phi,psi,t,p,rho1,rho2,what_time = "1:t/t",ind_tpts=FALSE, delta_y=FALSE) {
  ...
  delta <- kronecker(rho1^abs(outer(1:(p), 1:(p), FUN = `-`)),
...
simulateSDR <- function(nsim, n, phi, psi, t, p, rho1,rho2,H=1, d=1, k=1, npred = 100, what_time = "1:t/t",nlev_ydis = 10,
  ...
  new_X_red <- X_red 
  y_mse <- y_cor <- array(c(0),dim=c(5*k + 2+k,3,nsim),dimnames=list(names(X_red),c("lm","lm w.2.ord.terms","gam")))
...
#                       n,          phi p x m,                            psi txr, t, p, H, d, rho1,rho2, k,  time, ind_tpts, delta_y
#              --------------------------------------------------------
params <- list( #list( 500, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 3,"exp(1:t/6)/exp(t/6)", FALSE, TRUE),
                list( 500, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 3,"exp(1:t/6)/exp(t/6)", FALSE, FALSE),
                #list( 500, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 3,"runif(t)", TRUE, TRUE),
                list( 500, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 3,"runif(t)", TRUE, FALSE),
  #list( 2000, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 1,"exp(1:t/6)/exp(t/6)", FALSE, TRUE),
  list( 2000, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 1,"exp(1:t/6)/exp(t/6)", FALSE, FALSE),
  #list( 2000, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 1,"runif(t)", TRUE, TRUE),
  list( 2000, matrix(c(rep(c(0.1,-0.05),4),rep(c(1,-1),4)),nrow=8),matrix(c(1,0,1),nrow=3), 3, 8, 3, 2,   0.8,0.5, 1,"runif(t)", TRUE, FALSE)
)
...
for (param in params) {
  ...
  sim <- simulateSDR(nsim, n, phi, psi, t, p, rho1,rho2,H=H, d=d,k=k,npred=100,nlev_ydis = 10,what_time = time_sel,
                     ind_tpts = ind_tpts,delta_y = delta_y)

  saveRDS(sim, file = sprintf("../results/simulation_contLSIR_%d_%d_%d_%d_%d_%d_%d_%.2f_%.2f_%d_time_%.1s_ind_tpts_%s_delta_y_%s_kron.rds",
