...
simulateSDR <- function(nsim, n, B, t, p, rho,H=1, d=1, k=1, npred = n, what_time = "1:t/t",
  ...
  new_X_red <- X_red 
  predmodBIC <- AUC_est <- array(c(0),dim=c(5*k + 2+k,3,nsim),dimnames=list(names(X_red),c("logit","logit w.2.ord.terms","gam")))
  ...
  return(list(yAUC_mean = apply(AUC_est,1:2,mean), yAUC_sd=apply(AUC_est,1:2,sd),AUC_est=AUC_est,
              ...
              what_time=what_time,B=B,rkTest=rkTest
              ))
}
...
for (param in params) {
  ...
  saveRDS(sim, file = sprintf("../results/rank_binarySTIR_%d_%d_%d_%d_%d_%d_%d_%.2f_%d_time_%.1s_ind_tpts_%s_delta_y_%s.rds",
